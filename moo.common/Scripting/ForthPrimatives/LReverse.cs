    ...
    public static ForthPrimativeResult Execute(ForthPrimativeParameters parameters)
    {
        ...
        */
        if (parameters.Stack.Count < 1)
            return new ForthPrimativeResult(ForthErrorResult.STACK_UNDERFLOW, "LREVERSE requires at least one parameter");
        ...
        if (parameters.Stack.Count < i)
            return new ForthPrimativeResult(ForthErrorResult.STACK_UNDERFLOW, $"LREVERSE would reverse the top {Math.Abs(i)} items from the top of the stack, but only {parameters.Stack.Count} were present.");
