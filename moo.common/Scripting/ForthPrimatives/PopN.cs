...
public static class PopN
{
    public static ForthProgramResult Execute(ForthPrimativeParameters parameters)
    {
        ...
        if (parameters.Stack.Count < i)
            return new ForthProgramResult(ForthProgramErrorResult.STACK_UNDERFLOW, $"POPN would have removed {i} items on the stack, but only {parameters.Stack.Count} were present.");
